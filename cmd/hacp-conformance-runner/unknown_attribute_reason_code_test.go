...
func TestConformanceRunnerReportsBoundaryCrossingForOutOfScopeToolName(
	t *testing.T,
) {
	...
	const keyID = "unknown-attribute-reason-test-key"
	...
	unsignedEnvelope := map[string]any{
		"hacp_version":     "0.9",
		"envelope_id":      "unknown-attribute-reason-envelope",
		"principal":        "unknown-attribute-reason-system",
		"principal_kind":   "system",
		"intent_statement": "Out-of-scope tool_name reason-code conformance",
		"scope": map[string]any{
			"verbs":            []string{"execute"},
			"resource_classes": []string{"job"},
			"audiences":        []string{"internal"},
			"reversibility":    []string{"reversible"},
			"externality":      []string{"internal"},
			"data_classes":     []string{"internal"},
			"tool_names":       []string{"tool-a"},
		},
	...
	proposedAction := json.RawMessage(`{
				"hacp_version":"0.9",
				"action_id":"unknown-attribute-reason-action",
				"envelope_id":"unknown-attribute-reason-envelope",
				"verb":"execute",
				"resource_class":"job",
				"resource_id":"job://9",
	...
	resp := runner.HandleRequest(Request{
		...
		Operation:       "evaluate",
		VectorID:        "CORE-INV2-008-OUT-OF-SCOPE-TOOL-NAME-REASON",
		Input:           input,
	})
...
func TestConformanceRunnerReportsUnknownAttributeForAbsentToolName(
	t *testing.T,
) {
	publicKey, privateKey, err := ed25519.GenerateKey(rand.Reader)
	if err != nil {
	...
	unsignedEnvelope := map[string]any{
		...
		"principal":        "unknown-attribute-reason-system",
		"principal_kind":   "system",
		"intent_statement": "Absent tool_name reason-code conformance",
		"scope": map[string]any{
			"verbs":            []string{"execute"},
			"resource_classes": []string{"job"},
			"audiences":        []string{"internal"},
	...
	resp := runner.HandleRequest(Request{
		ProtocolVersion: ProtocolVersion,
		Operation:       "evaluate",
		VectorID:        "CORE-INV2-008-ABSENT-TOOL-NAME-REASON",
		Input:           input,
	})
	...
	const wantReason = "UNKNOWN_ATTRIBUTE"
